...
### 1 — Claim a Request
```bash
node /data/img-gen/img-claim.js img-gen-3
```
...
```bash
node -e "const c=JSON.parse(require('fs').readFileSync('/data/img-gen/google-accounts.json','utf8')); console.log(c['img-gen-3'].mlProfileId);"
```
...
1. Take screenshot: `{"action": "screenshot", "path": "/tmp/img-gen-3-debug.png"}`
