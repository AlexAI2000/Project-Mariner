...
### 1 — Claim a Request
```bash
node /data/img-gen/img-claim.js img-gen-5
```
...
```bash
node -e "const c=JSON.parse(require('fs').readFileSync('/data/img-gen/google-accounts.json','utf8')); console.log(c['img-gen-5'].mlProfileId);"
```
...
1. Take screenshot: `{"action": "screenshot", "path": "/tmp/img-gen-5-debug.png"}`
